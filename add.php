<?php
    ...
    if(isset($_POST['save'])){
        $_SESSION['list'] = $_POST['currentList'];
        ...
        if($t == "1"){ //twitter
            ...
            function getLinkId($urlX) {
                $urlX = explode("?s=", explode("/", $urlX)[5]);
                return $urlX[0];
            }
            
            //use this if twitter is still using the long link
            // $links  = substr($_POST['links'], 0, -25);

            $links  = $_POST['links'];
            $linksID = getLinkId($_POST['links']);
            //explode  the link to find the id
            
            $similarLinkTable = mysqli_query($connect,"SELECT * FROM links WHERE link LIKE '%$linksID%'");
            $similarLink = mysqli_fetch_array($similarLinkTable);
            if($similarLink == true){
                echo "<script>window.alert('This link is already in the system!')</script>";
                echo "<script>window.location.assign('?t=$t')</script>";
                //if duplicate was found, abort everything
                
                // header("location:add?t=$t");
                // echo "<script>if(window.alert('This link is already in the system!')){window.location.reload();}</script>";
                // var_dump($similarLink);
            }else{
                //trim the link to extract the artist name
                // $artist_tags = str_replace("/st", "", substr($_POST['links'], 20, -57));

                //explode the link to find the artist in the middle and add an @ into it
                $artist_tags = $_POST['links'];
                $artist_name = explode("/", $artist_tags)[3];
                $artist = "@".$artist_name."";

                //flick the switch to tell that the comparing process was a success
                $caseA = "1";  
            }
